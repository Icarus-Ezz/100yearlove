		...
		pcall(function()
			if _G.BringMonster then
				...
				for _, mob in pairs(workspace.Enemies:GetChildren()) do
					local isTargetMob = (mob.Name == Mon)
					local isSpecialMob = Mon == "Factory Staff [Lv. 800]" or Mon == "Monkey [Lv. 14]" or Mon == "Dragon Crew Warrior [Lv. 1575]" or Mon == "Dragon Crew Archer [Lv. 1600]"
					local player = game.Players.LocalPlayer
					local hrp = player.Character and player.Character:FindFirstChild("HumanoidRootPart")
					local mobHrp = mob:FindFirstChild("HumanoidRootPart")
					local mobHum = mob:FindFirstChild("Humanoid")

					if _G.Level and StartMagnet and isTargetMob and mobHrp and mobHum and mobHum.Health > 0 and hrp then
						local distance = (mobHrp.Position - hrp.Position).Magnitude
						local allowDistance = (isSpecialMob and distance <= 220) or (not isSpecialMob and distance <= (_G.BringMode or 450))
						if allowDistance then
							mobHrp.Size = Vector3.new(50, 50, 50)
							mobHrp.CFrame = PosMon
							mobHum:ChangeState(14)
							mobHrp.CanCollide = false
							if mob:FindFirstChild("Head") then
								mob.Head.CanCollide = false
							end
							local animator = mobHum:FindFirstChild("Animator")
							if animator then
								animator:Destroy()
							end
